...
public class LoginService implements ILoginService{
    ...
    public Optional<Login> registrarLogin(String correo, String contrasenia, Persona persona) {
        if (loginRepository.existsByCorreo(correo)) {
    ...
    @Override
    public Login login(String correo, String contrasenia) throws Exception {
    ...
    @Override
    public Login actualizarContrasenia(int idLogin, String nuevaContrasenia) {
        Login login = loginRepository.findById(idLogin)
                .orElseThrow(() -> new IllegalArgumentException("Login no encontrado"));
        login.setContrasenia(nuevaContrasenia);
